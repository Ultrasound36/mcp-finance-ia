...
class FinancialQuery(BaseModel):
    ...
    query_type: str = Field(
        ..., description="Tipo de consulta: price, analysis, forecast, comparison"
    )
    timeframe: Optional[str] = Field(
        "1mo",
        description="Periodo de tiempo: 1d, 5d, 1mo, 3mo, 6mo, 1y, 2y, 5y, 10y, ytd, max",
    )
...
class FinancialAnalyst:
    ...
    async def analyze_financial_data(self, data: Dict[str, Any], query: str) -> str:
        ...
        try:
            ...
            return response.choices[
                0
            ].message.content  # pyright: ignore[reportReturnType]
    ...
    def _detect_trend(self, data: Dict[str, Any]) -> str:
        ...
        if (
            "current_price" not in data
            or "high_52w" not in data
            or "low_52w" not in data
        ):
            return "INDEFINIDA"
    ...
    def _calculate_momentum(self, data: Dict[str, Any]) -> Dict[str, float]:
        ...
        return {
            ...
            "fuerza_tendencia": abs(data.get("daily_return_pct", 0))
            / (data.get("volatility_pct", 1) + 0.1),
        }
    ...
    def _analyze_volatility(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "riesgo": (
                "ALTO" if volatility > 30 else "MEDIO" if volatility > 15 else "BAJO"
            ),
            "stop_loss_sugerido": round(
                data.get("current_price", 0) * (1 - volatility / 100), 2
            ),
        }
    ...
    def _generate_recommendation(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        score += (
            2
            if trend in ["ALCISTA_FUERTE"]
            else (
                1
                if trend == "ALCISTA"
                else (
                    -2 if trend == "BAJISTA_FUERTE" else -1 if trend == "BAJISTA" else 0
                )
            )
        )
        ...
        score += (
            1
            if momentum["fuerza_tendencia"] > 0.5
            else -1 if momentum["fuerza_tendencia"] < -0.5 else 0
        )

        return {
            "accion": (
                "COMPRAR" if score >= 2 else "VENDER" if score <= -2 else "MANTENER"
            ),
...
class FinancialDataProvider:
    ...
    def get_currency_data(
        base: str, target: str, amount: float = 1.0
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "error": f"Error obteniendo datos de divisa {base}/{target}: {str(e)}"
            }
    ...
    for symbol in currencies:
        ...
        if "error" not in data:
            currency_data.append(
                {"pair": f"{base}/{target}", "rate": data["exchange_rate"]}
            )

    overview_data = {"indices": index_data, "currencies": currency_data}
